...
<?php
$id = check_string($_GET['id']);
$check = $DMH->get_row(" SELECT * FROM `danhmucmuacode` WHERE `id` = '$id' ");
if(!$check)
{
                    ...
                    <div id="app" data-v-app="">
                        <section>
                            ...
                            <div class="ant-row css-eq3tly" style="margin-left: -9px; margin-right: -9px;">
                                <?php foreach($DMH->get_list(" SELECT * FROM `danhsachmuacode` WHERE `id_danhmuc` = '$id' AND `hienthi` = 'SHOW' ORDER BY id DESC") as $row){ ?>
