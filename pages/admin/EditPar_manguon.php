...
if(isset($_GET['id']) && $getUser['level'] == 'admin')
{
    ...
    $row = $TUANORI->get_row(" SELECT * FROM `partner_code` WHERE `id` = '".$_GET['id']."' ");
    if(!$row) {
        echo msg_admin("error","Mã nguồn không tồn tại",BASE_URL('Admin/Par_manguon'), 1000); die;
    }
} else {
    echo msg_admin("error","Đường link không hợp lệ",BASE_URL('Admin/Par_manguon'), 1000); die;
}
?>
<h2>Edit mã nguồn <a href="/Admin/Par_manguon" class="btn btn-info"><i class="fa fa-backward"></i> Quay Lại</a></h2>
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-primary" data-collapsed="0">
            ...
            <div class="panel-body">
                <form role="form" class="form-horizontal form-groups-bordered">
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select id="danhmuc" class="selectboxit">
                                <?php foreach($TUANORI->get_list(" SELECT * FROM `danhmucmuacode` WHERE `status` = 'SHOW' ORDER BY id DESC") as $ok){ ?>
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label" style="color: green">Id public</label>
                        <div class="col-sm-5">
                            <input type="text" id="id_public" value="<?=$row['id_public'];?>" class="form-control daterange" />
                        </div>
                    </div>
                    <div class="form-group">
    ...
    <script type="text/javascript">
        $("#btnAdd").on("click", function() {
            $('#btnAdd').html('<i class="fa fa-spinner fa-spin"></i> Đang xử lý...').prop('disabled',
                true);
            $.ajax({
                url: "<?=BASE_URL('controller/admin/EditPar_manguon.php');?>",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: "<?=$row['id'];?>",
