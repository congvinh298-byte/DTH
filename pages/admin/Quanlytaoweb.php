...
{
    $user2 = $DMH->get_row(" SELECT * FROM `lichsutaoweb` WHERE `id` = '".$_GET['xoa']."'");
    if(!$user2) {
        echo msg("error", "Lịch sử tạo website không tồn tại", BASE_URL('Admin/Quanlytaoweb'), 1000); die;
    } else {
        $dele = $DMH->remove("lichsutaoweb", " `id` = '".$_GET['xoa']."' ");
        if($dele) {
...
<table class="table table-bordered responsive">
	...
	<tbody>
    <?php $i = 1;  foreach($DMH->get_list(" SELECT * FROM `lichsutaoweb` ORDER BY id DESC LIMIT 100") as $row){ ?>
        <tr>
            <td><?=$i++;?></td>
