...
if(isset($_GET['mokhoa']) && $getUser['level'] == 'admin') {
    $user2 = $TUANORI->get_row(" SELECT * FROM `users` WHERE `id` = '".$_GET['mokhoa']."'");
    if(!$user2) {
...
<table class="table table-bordered responsive">
	...
	<tbody>
		<?php $i = 0;  foreach($TUANORI->get_list(" SELECT * FROM `users` WHERE `banned` = 'OFF' ORDER BY id DESC LIMIT 50") as $row){ ?>
		<tr>
			<td><?=++$i;?></td>
