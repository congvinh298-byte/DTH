<?php
    ...
    if(!$TUANORI->site('client_id') || !$TUANORI->site('client_key')) {
        die("Chức năng Login Bằng Google Đã Bị Dừng");
    }
    require_once("../Google/vendor/autoload.php");
// Lấy những giá trị này từ https://console.cloud.google.com/apis/dashboard
$client_id = $TUANORI->site('client_id'); // ID GG
$client_secret = $TUANORI->site('client_key'); // SECRET GG
$redirect_uri = BASE_URL('LoginGoogle'); // LINK CALLBACK NHẬN DỮ LIỆU
...
if (isset($_GET['code'])) 
{
    ...
    $CheckUser = $TUANORI->get_row(" SELECT * FROM `users` WHERE `email` = '$email'");
    if($CheckUser)
    {
    ...
    {
        $timegio = time();
        $username = 'GG'.strtoupper(preg_replace('/[^a-zA-Z0-9_ -]/s', '', explode('@', $email)[0]));
