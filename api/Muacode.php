    ...
    if($api_keys) {
        if($api_keys['buy_code'] == 'ON') {
            ...
            if(isset($_GET['mgg'])) {
                ...
                $check_mgg = $DMH->get_row(" SELECT * FROM `magiamgia` WHERE `magiamgia` = '$mgg' AND `conlai` >  0 AND `theloai` = 'muacode' ");
                if(!$check_mgg) {
            ...
            $check_code = $DMH->get_row(" SELECT * FROM `danhsachmuacode` WHERE `id` = '$id_code' AND `hienthi` = 'SHOW' ");
            if(!$check_code) {
            ...
            if($block = $DMH->get_row(" SELECT * FROM `lichsumuacode` WHERE `username` = '".$check_api['username']."' ORDER BY id DESC ")) {
            ...
            if($isMoney) {
                ...
                if($create) {
                    if(isset($mgg)) {
                        $IsMgg  = $DMH->tru("magiamgia", "conlai", 1, " `magiamgia` = '$mgg'");
                        $IsMgg2 = $DMH->cong("magiamgia", "dasudung", 1, " `magiamgia` = '$mgg'");
                    }
                    $IsMgg3 = $DMH->cong("danhsachmuacode", "luottai", 1, " `id` = '$id_code'");
                    $IsMgg4 = $DMH->cong("danhsachmuacode", "luotxem", 1, " `id` = '$id_code'");
                    $arrDataPost = array(
                        'status'        => true,
