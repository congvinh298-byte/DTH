...
json_code(false, "Tạm thời không hoạt động");
...
if(!$check_api || $token !== $check_api['token_api']) {
    ...
    if($api_keys) {
        ...
        if(isset($_GET['danhmuc'])) {
            ...
            if(!$TUANORI->get_row(" SELECT * FROM `danhmuctaoweb` WHERE `id` = '$id_muc' ")) {
        ...
        if($api_keys['list_web'] == 'ON') {
            $get_list = $TUANORI->get_list(" SELECT * FROM `danhsachtaoweb` WHERE `hienthi` = 'SHOW' $muc ORDER BY id DESC");
            $user = $check_api;
            $list = [];
            ...
            json_code(false, "Chức năng LIST WEB của tài khoản này chưa được kích hoạt. Hãy truy cập vào link này: ".BASE_URL('Docs_api')." để kích hoạt nó");
        }
    } else {
