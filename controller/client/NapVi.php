...
if(isset($_COOKIE['token'])) {
    ...
    if($DMH->num_rows(" SELECT * FROM `hoadon_vi` WHERE`username` = '".$getUser['username']."' AND `status` IN ('xuly', 'huy') AND `thoigian` >= DATE(NOW()) AND `thoigian` < DATE(NOW()) + INTERVAL 1 DAY") > 5) {
        msg_error2("Bạn có nhiều hóa đơn trong hôm nay. Vui lòng quay lại vào ngày mai.");
    }
    $rand = random('QWERTYUIOPASDFGHJKLZXCVBNM', 8);
    $thucnhan = $sotien;
    if($DMH->site('sukien') == 'ON' && $DMH->site('khuyenmai') > 0) {
        $thucnhan +=$sotien*$DMH->site('khuyenmai')/100;
    }
    $DMH->insert("hoadon_vi", [
