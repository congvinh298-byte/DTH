...
if (isset($_POST['type']) && $_POST['type'] == 'PayPal' && isset($_POST['order']))
{
    ...
    $environment = new ProductionEnvironment($DMH->site('clientId'), $DMH->site('clientSecret'));
    ...
    try {
        ...
        if ($DMH->num_rows("SELECT * FROM `nappaypal` WHERE `trans_id` = '".$order->id."' ") > 0) {
        ...
        $price = $DMH->site('sotien_paypal') * $usd;
        $isInsert = $DMH->insert("nappaypal", [
        ...
        $DMH->cong("users", "money", $price, " `username` = '".$getUser['username']."' ");
        $DMH->cong("users", "total_money", $price, " `username` = '".$getUser['username']."' ");
        if ($isInsert) {
